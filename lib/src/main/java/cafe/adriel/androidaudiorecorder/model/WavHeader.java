...
public class WavHeader {
    ...
    byte bitPerSample;
    long sampleRateInHz;
    public WavHeader(long totalAudioLength, int audioFormat, byte bitPerSample, long sampleRateInHz) {
    ...
    public byte[] toBytes() {
        int channels =
                (audioFormat == AudioFormat.CHANNEL_IN_MONO ? 1 : 2);
        return wavFileHeader(totalAudioLength - 44, totalAudioLength - 44 + 36, sampleRateInHz,
                channels, bitPerSample * sampleRateInHz * channels / 8, bitPerSample);
    }
